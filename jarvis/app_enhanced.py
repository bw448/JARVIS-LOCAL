...
class EnhancedJarvisHTTPServer(ThreadingHTTPServer):
    ...
    def __init__(
        ...
        application: EnhancedJarvisApplication,
    ) -> None:
        self.application = application
        super().__init__(server_address, request_handler)


class EnhancedJarvisRequestHandler(BaseHTTPRequestHandler):
    server_version = "JarvisLocal/0.7.1"
    MAX_JSON_BYTES = 1024 * 1024

    @property
    def application(self) -> EnhancedJarvisApplication:
        server = self.server
        assert isinstance(server, EnhancedJarvisHTTPServer)
    ...
    def end_headers(self) -> None:
        ...
        # 添加性能监控头
        from .speech_optimizer import get_tts_cache
        cache_stats = get_tts_cache().stats()
        self.send_header("X-Cache-Hits", str(cache_stats.get("hits", 0)))
        self.send_header("X-Cache-Miss", str(cache_stats.get("misses", 0)))
    ...
    def _send_ndjson(self, events: Iterator[dict[str, Any]]) -> None:
        ...
        try:
            ...
            if isinstance(exc, (BrainError, ComputerUseError)):
                encoded = (
                    json.dumps(
...
def create_enhanced_server(
    host: str = "127.0.0.1",
    port: int = 8765,
    application: EnhancedJarvisApplication | None = None,
) -> EnhancedJarvisHTTPServer:
    if host not in {"127.0.0.1", "localhost", "::1"}:
        raise ValueError("JARVIS 目前只允许绑定本机回环地址")
    return EnhancedJarvisHTTPServer(
        (host, port), EnhancedJarvisRequestHandler, application or EnhancedJarvisApplication()
    )
